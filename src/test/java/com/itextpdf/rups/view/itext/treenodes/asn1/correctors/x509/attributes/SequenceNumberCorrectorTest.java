...
@Tag("UnitTest")
final class SequenceNumberCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        SequenceNumberCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "sequenceNumber");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        SequenceNumberCorrector.INSTANCE.correct(node, "SN");
