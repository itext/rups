...
@Tag("UnitTest")
final class OcspAcceptableResponsesCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OcspAcceptableResponsesCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "acceptableResponses");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OcspAcceptableResponsesCorrector.INSTANCE.correct(node, "oids");
        validateDefaultNode(node, "oids");
    }
    ...
    void correct_InvalidSequenceEntry() {
        ...
        OcspAcceptableResponsesCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(1, "acceptableResponses", node);
    ...
    private static void validateDefaultNode(AbstractAsn1TreeNode node, String expectedVariableName) {
        ...
        Asn1TestUtil.assertNodeMatches(
                0,
                "acceptableResponse: 1.3.6.1.5.5.7.48.1.1 (/iso/identified-organization/dod"
                        + "/internet/security/mechanisms/pkix/ad/id-ad-ocsp/id-pkix-ocsp-basic)",
                node.getChildAt(0)
        );
    }
