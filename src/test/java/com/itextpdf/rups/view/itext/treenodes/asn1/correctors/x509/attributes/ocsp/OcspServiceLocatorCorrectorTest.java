...
@Tag("UnitTest")
final class OcspServiceLocatorCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OcspServiceLocatorCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "serviceLocator");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OcspServiceLocatorCorrector.INSTANCE.correct(node, "locator");
        validateDefaultNode(node, "locator");
    }
    ...
    @Test
    void correct_InvalidRootSequenceTypes() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(
        ...
        OcspServiceLocatorCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(2, "serviceLocator", node);
    ...
    void correct_EmptySequence() {
        ...
        OcspServiceLocatorCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(0, "serviceLocator", node);
    }

    @Test
    void correct_TaggedRootSequenceTypes() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(
                new DERSequence(new ASN1Encodable[] {
    ...
    private static void validateDefaultNode(AbstractAsn1TreeNode node, String expectedVariableName) {
        ...
        {
            final AbstractAsn1TreeNode issuer = node.getChildAt(0);
            Asn1TestUtil.assertNodeMatches(1, "issuer", issuer);
            {
                final AbstractAsn1TreeNode rdn = issuer.getChildAt(0);
        ...
        {
            final AbstractAsn1TreeNode locator = node.getChildAt(1);
            Asn1TestUtil.assertNodeMatches(1, "locator", locator);
            {
                final AbstractAsn1TreeNode desc = locator.getChildAt(0);
                Asn1TestUtil.assertNodeMatches(2, "accessDescription", desc);
                Asn1TestUtil.assertNodeMatches(
                        0,
                        "accessMethod: 1.3.6.1.5.5.7.48.1 (/iso/identified-organization/dod"
                                + "/internet/security/mechanisms/pkix/ad/id-ad-ocsp)",
                        desc.getChildAt(0)
                );
                Asn1TestUtil.assertNodeMatches(
                        0,
                        "accessLocation: https://example.com/",
                        desc.getChildAt(1)
                );
            }
        }
