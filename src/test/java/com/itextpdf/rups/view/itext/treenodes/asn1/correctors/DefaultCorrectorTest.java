...
final class DefaultCorrectorTest {
    ...
    void correct_WithoutDefaultName() {
        ...
        DefaultCorrector.INSTANCE.correct(node, "value");
        Asn1TestUtil.assertNodeMatches(0, "value: NULL", node);
    }

    @Test
    void correct_WithObjAndDefaultName() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(
                DERNull.INSTANCE
        );
        DefaultCorrector.INSTANCE.correct(node, DERNull.INSTANCE);
        Asn1TestUtil.assertNodeMatches(0, "NULL: NULL", node);
    }
}