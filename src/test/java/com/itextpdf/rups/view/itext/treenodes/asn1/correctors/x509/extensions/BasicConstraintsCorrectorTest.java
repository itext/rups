...
@Tag("UnitTest")
final class BasicConstraintsCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        BasicConstraintsCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "basicConstraints");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        BasicConstraintsCorrector.INSTANCE.correct(node, "bc");
        validateDefaultNode(node, "bc");
    }
    ...
    void correct_EmptySequence() {
        ...
        BasicConstraintsCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(0, "basicConstraints", node);
    }
    ...
    void correct_InvalidSequenceTypes() {
        ...
        BasicConstraintsCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(2, "basicConstraints", node);
    ...
    private static void validateDefaultNode(AbstractAsn1TreeNode node, String expectedVariableName) {
        ...
        Asn1TestUtil.assertNodeMatches(0, "ca: TRUE", node.getChildAt(0));
