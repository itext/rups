...
@Tag("UnitTest")
final class TimeCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        TimeCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "time");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        TimeCorrector.INSTANCE.correct(node, "t");
        validateDefaultNode(node, "t");
    }

    @Test
    void correct_UtcTime() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(
                new DERUTCTime("240501133700")
        );
        TimeCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(0, "time: 240501133700 (2024-05-01T13:37:00Z)", node);
    }
    ...
    private static void validateDefaultNode(AbstractAsn1TreeNode node, String expectedVariableName) {
        Asn1TestUtil.assertNodeMatches(
                0,
                expectedVariableName + ": 202405011337 (2024-05-01T13:37:00Z)",
                node
        );
    }
