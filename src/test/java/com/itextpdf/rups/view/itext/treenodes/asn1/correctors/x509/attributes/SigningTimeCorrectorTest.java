...
@Tag("UnitTest")
final class SigningTimeCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        SigningTimeCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "signingTime");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        SigningTimeCorrector.INSTANCE.correct(node, "ST");
        validateDefaultNode(node, "ST");
    }
    ...
    void correct_UtcTime() {
        ...
        SigningTimeCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(0, "signingTime: 240501133700 (2024-05-01T13:37:00Z)", node);
    }
