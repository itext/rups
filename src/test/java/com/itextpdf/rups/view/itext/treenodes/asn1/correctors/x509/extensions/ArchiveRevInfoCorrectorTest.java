...
@Tag("UnitTest")
final class ArchiveRevInfoCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        ArchiveRevInfoCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "archiveRevInfo");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        ArchiveRevInfoCorrector.INSTANCE.correct(node, "ARI");
        validateDefaultNode(node, "ARI");
    }
    ...
    void correct_EmptySequence() {
        ...
        ArchiveRevInfoCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(0, "archiveRevInfo", node);
    }
    ...
    void correct_InvalidSequenceEntry() {
        ...
        ArchiveRevInfoCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(1, "archiveRevInfo", node);
