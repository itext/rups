...
@Tag("UnitTest")
final class ContentTypeCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        ContentTypeCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "contentType");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        ContentTypeCorrector.INSTANCE.correct(node, "CT");
        validateDefaultNode(node, "CT");
    }
    ...
    private static void validateDefaultNode(AbstractAsn1TreeNode node, String expectedVariableName) {
        Asn1TestUtil.assertNodeMatches(
                0,
                expectedVariableName + ": 1.2.840.113549.1.7.2 (/iso/member-body/us/rsadsi/pkcs"
                        + "/pkcs-7/signedData)",
                node
        );
    }
