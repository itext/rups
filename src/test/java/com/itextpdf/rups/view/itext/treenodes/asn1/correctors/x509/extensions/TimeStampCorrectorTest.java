...
@Tag("UnitTest")
final class TimeStampCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        TimeStampCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "timeStamp");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        TimeStampCorrector.INSTANCE.correct(node, "TS");
        validateDefaultNode(node, "TS");
    }
    ...
    void correct_EmptySequence() {
        ...
        TimeStampCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(0, "timeStamp", node);
    }
    ...
    void correct_InvalidSequenceTypes() {
        ...
        TimeStampCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(3, "timeStamp", node);
    ...
    void correct_InvalidVersionNumber() {
        ...
        TimeStampCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(1, "timeStamp", node);
    ...
    private static void validateDefaultNode(AbstractAsn1TreeNode node, String expectedVariableName) {
        ...
        Asn1TestUtil.assertNodeMatches(0, "location: http://adobe-timestamp.geotrust.com/tsa", node.getChildAt(1));
    ...
    private AbstractAsn1TreeNode createDefaultNode() {
        return Asn1TreeNodeFactory.fromPrimitive(
                new DERSequence(new ASN1Encodable[] {
                        new ASN1Integer(1),
                        new DERTaggedObject(false, 6, new DERIA5String("http://adobe-timestamp.geotrust.com/tsa")),
                        ASN1Boolean.TRUE
                })
        );
    }
