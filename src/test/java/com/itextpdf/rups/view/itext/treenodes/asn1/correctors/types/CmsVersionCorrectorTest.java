...
@Tag("UnitTest")
final class CmsVersionCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(new ASN1Integer(0));
        CmsVersionCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(0, "version: 0 (v0)", node);
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(new ASN1Integer(0));
        CmsVersionCorrector.INSTANCE.correct(node, "v");
        Asn1TestUtil.assertNodeMatches(0, "v: 0 (v0)", node);
    }
    ...
    private static void testVersion(int value, String description) {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(new ASN1Integer(value));
        CmsVersionCorrector.INSTANCE.correct(node);
        String expectedString = "version: " + value;
        if (description != null) {
            expectedString += " (" + description + ")";
        }
        Asn1TestUtil.assertNodeMatches(0, expectedString, node);
    }
}
