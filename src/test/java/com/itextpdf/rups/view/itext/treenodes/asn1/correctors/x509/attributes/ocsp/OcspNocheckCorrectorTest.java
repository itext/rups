...
@Tag("UnitTest")
final class OcspNocheckCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OcspNocheckCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "nocheck");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OcspNocheckCorrector.INSTANCE.correct(node, "noCheck");
        validateDefaultNode(node, "noCheck");
    }
