...
@Tag("UnitTest")
final class RevocationInfoChoicesCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        RevocationInfoChoicesCorrector.INSTANCE.correct(node);
    ...
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        RevocationInfoChoicesCorrector.INSTANCE.correct(node, "revLists");
        validateDefaultNode(node, "revLists");
    }
    ...
    void correct_EmptyRootSet() {
        ...
        RevocationInfoChoicesCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(0, "crls", node);
    }
    ...
    void correct_InvalidRevocationInfoChoiceType() {
        ...
        RevocationInfoChoicesCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(1, "crls", node);
    ...
    void correct_InvalidOtherRevocationInfoFormatBaseType() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(
                new DERSet(new DERTaggedObject(false, 1, new ASN1Integer(1)))
        );
        RevocationInfoChoicesCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(1, "crls", node);
    ...
    void correct_EmptyOtherRevocationInfoFormatSequence() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(
                new DERSet(new DERTaggedObject(false, 1, new DERSequence()))
        );
        RevocationInfoChoicesCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(1, "crls", node);
    ...
    @Test
    void correct_InvalidOtherRevInfoFormatType() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(
                new DERSet(
    ...
    private static void validateDefaultNode(AbstractAsn1TreeNode node, String expectedVariableName) {
        ...
        {
            final AbstractAsn1TreeNode crl = node.getChildAt(0);
            Asn1TestUtil.assertNodeMatches(3, "crl", crl);
            Asn1TestUtil.assertNodeMatches(0, "tbsCertList", crl.getChildAt(0));
            {
                final AbstractAsn1TreeNode signatureAlgorithm = crl.getChildAt(1);
            ...
            Asn1TestUtil.assertNodeMatches(0, "signatureValue: 0x1223344556", crl.getChildAt(2));
        }
        {
            final AbstractAsn1TreeNode crl = node.getChildAt(1);
            Asn1TestUtil.assertNodeMatches(2, "other", crl);
            Asn1TestUtil.assertNodeMatches(
                    0, "otherRevInfoFormat: 1.2 (/iso/member-body)", crl.getChildAt(0)
            );
        ...
        return Asn1TreeNodeFactory.fromPrimitive(
                new DLSet(new ASN1Encodable[] {
                        new DERSequence(new ASN1Encodable[] {
                                // Not filling here, since CertificateList is covered separately
                                new DERSequence(),
                                new DERSequence(new ASN1Encodable[] {
                                        // sha256WithRSAEncryption
                                ...
                                new DERBitString(new byte[] {0x12, 0x23, 0x34, 0x45, 0x56}),
                        }),
