...
@Tag("UnitTest")
final class OcspExtendedRevokeCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OcspExtendedRevokeCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "extendedRevoke");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OcspExtendedRevokeCorrector.INSTANCE.correct(node, "revoke");
        validateDefaultNode(node, "revoke");
    }
