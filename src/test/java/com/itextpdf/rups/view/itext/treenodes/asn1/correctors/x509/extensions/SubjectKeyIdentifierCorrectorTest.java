...
@Tag("UnitTest")
final class SubjectKeyIdentifierCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        SubjectKeyIdentifierCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "subjectKeyIdentifier");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        SubjectKeyIdentifierCorrector.INSTANCE.correct(node, "SKI");
        validateDefaultNode(node, "SKI");
    }
