...
@Tag("UnitTest")
final class MessageDigestCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        MessageDigestCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "messageDigest");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        MessageDigestCorrector.INSTANCE.correct(node, "digest");
        validateDefaultNode(node, "digest");
    }
