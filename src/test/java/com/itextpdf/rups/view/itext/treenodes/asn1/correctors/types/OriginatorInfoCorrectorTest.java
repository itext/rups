...
final class OriginatorInfoCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OriginatorInfoCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "originatorInfo");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OriginatorInfoCorrector.INSTANCE.correct(node, "oi");
        validateDefaultNode(node, "oi");
    }
    ...
    void correct_EmptyRootSequence() {
        ...
        OriginatorInfoCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(0, "originatorInfo", node);
    }

    @Test
    void correct_WithUniversalField() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(
                new DERSequence(new ASN1Integer(23))
        );
        OriginatorInfoCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(1, "originatorInfo", node);
        Asn1TestUtil.assertNodeMatches(0, "INTEGER: 23", node.getChildAt(0));
    }
    ...
    private static void validateDefaultNode(AbstractAsn1TreeNode node, String expectedVariableName) {
        Asn1TestUtil.assertNodeMatches(2, expectedVariableName, node);
        {
            ...
            Asn1TestUtil.assertNodeMatches(0, "v1AttrCert", certs.getChildAt(0));
        }
        {
            final AbstractAsn1TreeNode crls = node.getChildAt(1);
