...
@Tag("UnitTest")
final class ContentInfoCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        ContentInfoCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "contentInfo");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        ContentInfoCorrector.INSTANCE.correct(node, "CI");
    ...
    void correct_EmptyRootSequence() {
        ...
        ContentInfoCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(0, "contentInfo", node);
    }
    ...
    void correct_InvalidRootSequenceFieldsTypes() {
        ...
        ContentInfoCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(2, "contentInfo", node);
    ...
    private static void validateDefaultNode(AbstractAsn1TreeNode node, String expectedVariableName) {
        ...
        );
        {
            ...
            Asn1TestUtil.assertNodeMatches(0, "digestAlgorithms", content.getChildAt(1));
            {
                ...
                Asn1TestUtil.assertNodeMatches(1, "encapContentInfo", eci);
                Asn1TestUtil.assertNodeMatches(
                        0,
                        "eContentType: 1.2.840.113549.1.7.1 (/iso/member-body/us/rsadsi/pkcs/pkcs-7"
                                + "/data)",
                        eci.getChildAt(0)
                );
            }
        ...
        return Asn1TreeNodeFactory.fromPrimitive(
                new DERSequence(new ASN1Encodable[] {
                        ...
                        new DERTaggedObject(true, 0, new DERSequence(new ASN1Encodable[] {
                                ...
                                new DERSet(),
                                new DERSequence(
                                        new ASN1ObjectIdentifier("1.2.840.113549.1.7.1")    // data
                                ),
                                new DERSet(),
                        })),
                })
