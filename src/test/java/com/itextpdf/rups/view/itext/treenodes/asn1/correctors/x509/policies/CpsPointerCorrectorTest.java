...
@Tag("UnitTest")
final class CpsPointerCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        CpsPointerCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "cpsUri");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        CpsPointerCorrector.INSTANCE.correct(node, "uri");
        validateDefaultNode(node, "uri");
    }
