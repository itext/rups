...
@Tag("UnitTest")
final class OcspArchiveCutoffCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OcspArchiveCutoffCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "archiveCutoff");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        OcspArchiveCutoffCorrector.INSTANCE.correct(node, "arch");
        validateDefaultNode(node, "arch");
    }
