...
@Tag("UnitTest")
final class AuthorityKeyIdentifierCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        AuthorityKeyIdentifierCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "authorityKeyIdentifier");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        AuthorityKeyIdentifierCorrector.INSTANCE.correct(node, "AKI");
        validateDefaultNode(node, "AKI");
    }
    ...
    void correct_EmptySequence() {
        ...
        AuthorityKeyIdentifierCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(0, "authorityKeyIdentifier", node);
    }
    ...
    void correct_InvalidSequenceTypes() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(
                new DERSequence(new ASN1Encodable[] {
                        ...
                        new DERPrintableString("C"),
                })
        );
        AuthorityKeyIdentifierCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(3, "authorityKeyIdentifier", node);
        ...
        Asn1TestUtil.assertNodeMatches(0, "PrintableString: C", node.getChildAt(2));
    }

    @Test
    void correct_InvalidKeyIdentifierBaseType() {
        final AbstractAsn1TreeNode node = Asn1TreeNodeFactory.fromPrimitive(
                new DERSequence(
        ...
        AuthorityKeyIdentifierCorrector.INSTANCE.correct(node);
        Asn1TestUtil.assertNodeMatches(1, "authorityKeyIdentifier", node);
        Asn1TestUtil.assertNodeMatches(0, "[0] IMPLICIT SEQUENCE", node.getChildAt(0));
    }
    ...
    private static void validateDefaultNode(AbstractAsn1TreeNode node, String expectedVariableName) {
        ...
        Asn1TestUtil.assertNodeMatches(0, "keyIdentifier: 0x010203", node.getChildAt(0));
        {
            final AbstractAsn1TreeNode authorityCertIssuer = node.getChildAt(1);
            Asn1TestUtil.assertNodeMatches(1, "authorityCertIssuer", authorityCertIssuer);
            Asn1TestUtil.assertNodeMatches(0, "ipAddress: 0x7F000001", authorityCertIssuer.getChildAt(0));
        }
        ...
        return Asn1TreeNodeFactory.fromPrimitive(
                new DERSequence(new ASN1Encodable[] {
                        new DERTaggedObject(false, 0,
                                new DEROctetString(new byte[] {0x01, 0x02, 0x03})
                        ),
