...
@Tag("UnitTest")
final class VersionCorrectorTest {
    @Test
    void correct_WithDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        VersionCorrector.INSTANCE.correct(node);
        validateDefaultNode(node, "version");
    }

    @Test
    void correct_WithoutDefaultName() {
        final AbstractAsn1TreeNode node = createDefaultNode();
        VersionCorrector.INSTANCE.correct(node, "v");
        validateDefaultNode(node, "v");
    }
    ...
    private static void testVersion(int value, String description) {
        ...
        VersionCorrector.INSTANCE.correct(node);
        if (description == null) {
            Asn1TestUtil.assertNodeMatches(0, "version: " + value, node);
        } else {
            Asn1TestUtil.assertNodeMatches(0, "version: " + value + " (" + description + ")", node);
        }
    }
}