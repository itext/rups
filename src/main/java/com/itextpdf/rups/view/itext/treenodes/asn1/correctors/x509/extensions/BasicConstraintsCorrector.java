...
/**
 * Corrector for the basicConstraints extension attribute, as it is
 * defined in RFC 5280.
 *
 * <pre>
 ...
 * </pre>
 */
public final class BasicConstraintsCorrector extends AbstractCorrector {
    /**
     * Singleton instance of the corrector.
     */
    public static final BasicConstraintsCorrector INSTANCE = new BasicConstraintsCorrector();

    private BasicConstraintsCorrector() {
        // singleton class
    }
    ...
    public void correct(AbstractAsn1TreeNode node, ASN1Primitive obj, String variableName) {
        ...
        if (node.getChildCount() > i) {
            final AbstractAsn1TreeNode ca = node.getChildAt(i);
            if (isUniversalType(ca, ASN1Boolean.class)) {
                ca.setRfcFieldName("ca");
                ++i;
            }
        }
        if (node.getChildCount() > i) {
            correctPrimitiveUniversalType(node.getChildAt(i), ASN1Integer.class, "pathLenConstraint");
        }
    }
}
